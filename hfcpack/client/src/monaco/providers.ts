...
import type { IGrammar, IOnigLib, IRawGrammar, IRawTheme, StackElement } from 'vscode-textmate'
...
class TokensProviderCache {
  ...
  async createEncodedTokensProvider(
    ...
    return {
      ...
      // this is for html emmet plugink, current only support start of line
      tokenize(
        line: string,
        ...
        if (scopeName === 'text.html.basic' && tokens.length === 1 && tokens[0].scopes[0] === 'text.html.basic')
          return { tokens: [{ scopes: 'text.html.basic', startIndex: 0 }], endState }
