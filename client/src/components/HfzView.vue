...
let viewportWidth = 'auto'
function toggleViewportWidth() {
  if (viewportWidth === 'auto')
    viewportWidth = '320px'

  else if (viewportWidth === '320px')
    viewportWidth = '414px'

  else if (viewportWidth === '414px')
    viewportWidth = '768px'

  else if (viewportWidth === '768px')
...
<template>
  ...
  <div class="mt-5 mb-10 rounded-md">
    <div class="action-bar flex rounded-t-md overflow-hidden bg-slate-700">
      <button
        class="flex items-center justify-center p-2 h-9 w-9 text-slate-300 hover:text-slate-100" title="Reload"
        @click="onReload"
      >
      ...
      <button
        class="flex items-center justify-center p-2 h-9 w-9 text-slate-300 hover:text-slate-100"
        title="Change Viewport Width" @click="toggleViewportWidth"
      >
        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24">
          <path
            fill="currentColor"
